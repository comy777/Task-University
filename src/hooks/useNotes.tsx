...
import { DataNoteSave, RequestGet, DataTaskSave, ActiveData } from '../interfaces/data';
...
const useNotes = () => {
	...
	const { title, body, handleChangeText, reset, resetFormValues } = useForm({ title: '', body: '' });
	const {
		images,
		getImagesCamera,
		getImagesGalery,
	...
	const handleSaveNote = async (id: string, data: DataNoteSave, note?: Note) => {
	...
	const handleSaveTask = async (id: string, data: DataTaskSave, task?: Task) => {
	...
	const handleSave = async (id: string, note?: Note, task?: Task, saveTask?: boolean) => {
		let data: DataNoteSave = { title, body, images: [] };
		let dataTask: DataTaskSave = { title, body, images: [], dayLimit: datePicker ? datePicker : undefined };
		setLoading();
		if (images.length > 0) {
	...
	const handleEdit = async (id: string, data: DataNoteSave | DataTaskSave, saveTask?: boolean) => {
	...
	const handleShowDelete = () => {
		if (activeData) {
			if (activeData.note) {
				showAlert({ title: 'Borrar', message: 'Seguro que quiere borrar la nota?', onPress: handleDelete });
			}
			if (activeData.task) {
				showAlert({ title: 'Borrar', message: 'Seguro que quiere borrar la tarea?', onPress: handleDelete });
			}
		}
	};
	const handleNavigateEdit = () => {
		...
		setModal();
		const { note, task } = activeData;
		if (note) navigation.navigate('create note', { note, id: note._id });
		if (task) navigation.navigate('create task', { task, id: task._id });
	};
	...
	const handleComplete = async (id: string) => {
		...
		setModal();
		if (!resp) return;
		const newTasks = tasks.map((item) => (item._id === id ? { ...item, complete: !item.complete } : item));
		setTasks(newTasks);
	};
