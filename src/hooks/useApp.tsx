...
import { appDeleteRequest, appGetLessons, appPostLessons, appPutLessons, authUserRequest } from '../api/request';
import useStyles from './useStyles';
import useForm from './useForm';
import { formDataApp, validateLesson, getDaysValidate, setDataDays } from '../utils/validate';
...
const useApp = (data?: Lesson) => {
	...
	const { lesson, teacher, nrc, handleChangeText, reset, resetFormValues } = useForm(form);
	...
	const handleChangeDay = (day: string) => {
		const validate = setDataDays({ days, day, setDays, schedlue, selected: true });
		if (validate) return;
		setDay(day);
		setChildren('lesson');
		setModal();
	...
	const handleSaveLesson = async () => {
		...
		const validate = validateLesson({ lesson, teacher, nrc, schedlue: dataDays });
		if (!validate) return;
		setLoading();
		if (data) {
	...
	const handleEdit = async (lesson: DataLessonSave) => {
		...
		setLoading();
		if (!resp) return;
		const newLessons = lessons.map((item) => (item._id === resp._id ? resp : item));
	...
	const showHandleDelete = () => {
		showAlert({ title: 'Borrar', message: 'Seguro que desea borrar esta clase?', onPress: handleDelete });
	};
	const handleActive = (data: Lesson) => {
		const { lesson, teacher, nrc, schedlue } = data;
