...
const useUser = () => {
	const { setToken, setEditable, editable, user, setUser, appState, setLoadingAuth } = useContext(AppContext);
	const {
		image,
		setImage,
		getImagesCamera,
	...
	const { username, handleChangeText } = useForm({ username: user ? user.username : '' });
