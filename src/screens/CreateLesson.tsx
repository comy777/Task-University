...
const CreateLesson = ({ route }: LessonScreenProps) => {
	const {
		...
		handleNavigate,
		days
	} = useApp(route.params ? (route.params.lesson ? route.params.lesson : undefined) : undefined);
	useEffect(
		() => {
			if (route.params) {
	...
	return (
		<View style={styles.container}>
			...
			<Fab icon="add" style={styles.fabContainerBottom} onPress={handleSaveLesson} loading={loading} />
		</View>
	);
};
