import { DataAuth, DataLesson, ValidateDays, DataNoteSave, DataTaskSave } from '../interfaces/data';
...
export const formDataAuth = (value: string) => {
	const form =
		value === 'login' ? { email: '', password: '' } : { email: '', password: '', repeatPassword: '', username: '' };
	return form;
};
...
export const validateLesson = ({ lesson, teacher, nrc, schedlue }: DataLesson) => {
	if (!lesson && !teacher && !nrc && !schedlue) return false;
	if (!lesson) {
	...
	return { lesson, teacher, nrc, schedlue };
};

export const setDataDays = ({ days, day, setDays, schedlue, selected }: ValidateDays) => {
	let bandera = false;
	if (selected) {
		days.map((item) => {
			if (item.day === day) {
				if (item.selected) {
					const newDays = days.map(
						(item) => (item.day === day ? { ...item, selected: !item.selected } : item)
					);
					setDays(newDays);
					bandera = true;
				}
		...
		const newDays = days.map((item) => (item.day === day ? { ...item, selected: true, schedlue } : item));
		setDays(newDays);
	}
	return bandera;
};
